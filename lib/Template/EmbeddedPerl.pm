...
use Template::EmbeddedPerl::Exception;
...
has [qw(auto_escape vars)] => (
  is => 'rw',
);

has compiled => (
  is => 'rw',
  clearer => '_clear_compiled',
);
...
has namespace => (
  is => 'rw',
  default => 'Template::EmbeddedPerl::Sandbox',
);
...
has escape => (
  ...
  default => sub {
    return sub {
      return "$_[0]" if ($_[0]->$_isa('Template::EmbeddedPerl::Raw'));
...
sub parse {
  ...
  # Clean start
  my @tree;
  $self->unparsed($template);
  $self->tree(\@tree);
  $self->_clear_compiled;

  # $self->tree(\my @tree)->compiled(undef);
...
sub process {
  ...
  unless ($compiled) {
    ...
    $compiled = eval $self->_wrap($code, @_);

    unless ($compiled) {
      my $e = Template::EmbeddedPerl::Exception->new($@);
      $e->inspect($self->unparsed, $code);
      $e->trace;
      $e->verbose(1);
      return $e;
    }
  ...
  local $SIG{__DIE__} = sub {
    ...
    my $e = Template::EmbeddedPerl::Exception->new(shift);
    $e->trace;
    $e->inspect($self->unparsed, $self->code);
    $e->verbose(1);
    CORE::die $e;
  };
...
sub render {
  my ($self, $template, @rest) = @_;
  my $parsed = $self->parse($template);
  $self->process(@rest) }
...
sub render_file {
  ...
  $template = eval { decode($encoding, $template, Encode::FB_CROAK) };
  croak qq{Template "$path" has invalid encoding: $@} if $@;
...
sub _compile {
  ...
  while (++$i <= @$tree && (my $next = $tree->[$i])) {
    ...
    # Capture end
      ...
      # XXX old code
      $blocks[-1] .= 'return Template::EmbeddedPerl::Raw->new($_O) }';
      # $blocks[-1] .= 'return $_O }';
...
sub _wrap {
  ...
  $code .= "use strict; no warnings 'ambiguous';";
...
package Template::EmbeddedPerl::Raw {
  sub new {
    my ($class, $str) = @_;
  ...
  sub to_string { return $_[0]->{string} };
}
...
Template::EmbeddedPerl - Mojo's Perl-ish templates, minus the Mojo
